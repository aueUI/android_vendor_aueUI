...
func init() {
	android.RegisterModuleType("aueUI_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aueUIExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aueUIExpandVariables(ctx, dep_file)
	...
	cmd := aueUIExpandVariables(ctx, String(g.properties.Cmd))
